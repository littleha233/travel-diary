...
Scope: REST API contract plus local Spring Boot backend MVP. Core write-chain behavior is implemented for trip/check-in/nearby flows, image upload supports local fallback plus MinIO/S3-compatible presigned PUT URLs, AI memory generation supports a backend provider abstraction for mock/Anthropic/DeepSeek modes, and Phase 5 wishlist/manual-light/plans are implemented with user-scoped runtime state. Persistence phase 6a now saves/restores runtime state through a database snapshot and projects it into the V2 core tables; production-grade domain mapper reads/writes and SMS are still follow-up work.
